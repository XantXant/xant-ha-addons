...
class Awattar:
    ...
    def getNewData(self):
        ...
        if act_time > self.time_to_get_new_data:
            ...
            self.time_to_get_new_data = self.time_of_data + datetime.timedelta(hours=1)

            # end datum für abfrage +2h
            url_date = self.time_of_data + datetime.timedelta(days=2)
            ...
            if self.test is True:
                ...
                url = f"https://api.awattar.at/v1/marketdata?end={int(url_date.timestamp()*1000)}"
                # print(url)
                webdata = urlopen(url)
